...
//        redirect('access/add_new_product');
    ...
    public function selling_product_pre(){
    ...
    public function selling_product(){
        ...
        $data['invoice_no'] = $invoice_no;
        $data['total_price'] = $this->input->post('total_price');
        $dis_percentage = $this->input->post('discount');
        $dis_reference = $this->input->post('discount_reference');
        $nt_price = $this->input->post('net_price');

        $data['total_price'] = $this->input->post('total_price');
        $data['net_price'] = $this->input->post('total_net_price');
        ...
        foreach ($product_id_array as $k => $v){
            $prod_code = $product_code_array[$k];
            $discount_percentage = $dis_percentage[$k];
            $discount_reference = $dis_reference[$k];
            $net_price = $nt_price[$k];
            $sale_date_time = $date_time;
            $invoice_id = $inserted_id;
            $sold_by_user_id = $user_id;

            $this->access_model->productSaleUpdate($prod_code, $sale_date_time, $invoice_id, $sold_by_user_id, $discount_percentage, $discount_reference, $net_price);
        }
    ...
    public function getProductInfoByCode(){
        ...
        $total_row_count = $this->input->post('total_row_count');
        ...
        if($prod_code != ''){
            ...
            $new_line .= '<td class="center"><input type="text" class="price" id="sale_price'.$total_row_count.'" name="sale_price[]" value="'.$sale_price.'" readonly /></td>';
            $new_line .= '<td class="center"><input type="text" class="discount" id="discount'.$total_row_count.'" name="discount[]" value="" onblur="getNetPrice('.$total_row_count.');" /></td>';
            $new_line .= '<td class="center"><input type="text" class="net_price" id="net_price'.$total_row_count.'" name="net_price[]" value="'.$sale_price.'" readonly /></td>';
            $new_line .= '<td class="center"><input type="text" class="reference" id="discount_reference'.$total_row_count.'" name="discount_reference[]" value="" readonly /></td>';
    ...
    public function getProductInfoByCodeReturn(){
        $shop = $this->session->userdata('shop');
        ...
        if($prod_code != ''){
            ...
            $new_line .= '<td class="center">'.$item_name.'</td>';
            $new_line .= '<td class="center">'.$size.'</td>';
            $new_line .= '<td class="center"><input type="text" class="price" id="purchase_price'.$total_row_count.'" name="purchase_price[]" value="'.$purchase_price.'" readonly /></td>';
            $new_line .= '<td><span class="btn btn-danger" id="remove" onclick="deleteRow(this);">REMOVE</span></td>';
    ...
    public function update_invoice(){
        ...
        $nt_price = $this->input->post('net_price');
        $discount_ref = $this->input->post('discount_reference');
        ...
        foreach ($product_id_array as $k => $v){
            $prod_code = $product_code_array[$k];
            $discount_percentage = $discount[$k];
            $net_price = $nt_price[$k];
            $discount_reference = $discount_ref[$k];
            $sale_date_time = $date_time;
            $sold_by_user_id = $user_id;
        ...
        $return_sale_price = $this->input->post('return_sale_price');
        $return_discount_percentage = $this->input->post('return_discount_percentage');
        $return_net_price = $this->input->post('return_net_price');
        $return_discount_reference = $this->input->post('return_discount_reference');

        foreach ($return_product_id as $k_1 => $rp) {
            $data1 = array(
                'invoice_id' => $invoice_id,
                'product_id' => $rp,
                'return_date_time' => $date_time,
                'returned_by' => $user_id,
                'sale_price' => $return_sale_price[$k_1],
                'discount_percentage' => $return_discount_percentage[$k_1],
                'discount_reference' => $return_discount_reference[$k_1],
                'net_price' => $return_net_price[$k_1]
            );
    ...
    public function editInvoice_pre($invoice_code, $invoice_id){
        ...
        $data1['maincontent'] = $this->load->view('edit_invoice_pre', $data1, true);
    ...
    public function purchase_report(){
        ...
        $data['maincontent'] = $this->load->view('purchase_report', $data, true);
        $this->load->view('master', $data);
    }
    ...
    public function getPurchaseReport(){
        ...
        if($from_date != '' && $from_date != '1970-01-01' && $to_date != '' && $to_date != '1970-01-01'){
            $where .= " AND DATE_FORMAT(t1.purchase_date, '%Y-%m-%d') between '$from_date' and '$to_date' ";
        }

        $data['purchase_report'] = $this->access_model->getPurchaseReport($where);

        echo $maincontent = $this->load->view('purchase_report_filter', $data);
    ...
    public function getInventoryReport(){
        ...
        if($item != ''){
            $where .= " AND t1.item_id='$item'";
        }

        if($size != ''){
            $where .= " AND t1.size_id='$size'";
        }

        if($from_date != '' && $from_date !='undefined--undefined' && $from_date != '1970-01-01' && $to_date != '' && $to_date !='undefined--undefined' && $to_date != '1970-01-01'){
            $where .= " and t1.purchase_date between '$from_date' and '$to_date' ";
        }

        $data['inventory_report'] = $this->access_model->getInventoryReport($where);

        echo $maincontent = $this->load->view('inventory_report_filter', $data);
    ...
    public function getSummaryReport(){
        ...
        $dt_where = '';
        if($till_date != '' && $till_date !='undefined--undefined' && $till_date != '1970-01-01'){
            $dt_where .= " and DATE_FORMAT(sale_date_time, '%Y-%m-%d') <= '$till_date' AND sale_date_time != '0000-00-00 00:00:00'";
        }

        $data['summary_report'] = $this->access_model->getSummaryReport($where, $dt_where);

        echo $maincontent = $this->load->view('summary_report_filter', $data);
    ...
    public function summary_report(){
        ...
        $data['maincontent'] = $this->load->view('summary_report', $data, true);
        $this->load->view('master', $data);
    }
    ...
    public function inventory_report(){
        ...
        $data['maincontent'] = $this->load->view('inventory_report', $data, true);
        $this->load->view('master', $data);
    }

    public function vendor_return_log_report(){
        $data['title'] = 'Vendor Return Log Report';
        ...
        if($month_year != '' && $month_year != '1970-01'){
            $where .= " AND DATE_FORMAT(t1.return_date_time, '%Y-%m')='$month_year'";
        }

        $data['shops'] = $this->access_model->getShops();
        $data['profit_loss_report'] = $this->access_model->getVendorReturnLogReport($where);

        $data['maincontent'] = $this->load->view('vendor_return_log_report', $data, true);
        $this->load->view('master', $data);
    }

    public function getVendorReturnLogReport(){
        $shop = $this->input->post('shop');
        $from_date = $this->input->post('from_date');
        $to_date = $this->input->post('to_date');
        ...
        if($from_date != '' && $from_date != '1970-01-01' && $to_date != '' && $to_date != '1970-01-01'){
            $where .= " and DATE_FORMAT(t1.return_date_time, '%Y-%m-%d') between '$from_date' and '$to_date' ";
        }
        $data['profit_loss_report'] = $this->access_model->getVendorReturnLogReport($where);

        echo $maincontent = $this->load->view('vendor_return_log_report_filter', $data);
...
//        $this->load->view('master', $data1);
    ...
    public function sale_product_pre(){
        $data1['title'] = 'Sale Product';
        $data1['user_name'] = $this->session->userdata('user_name');
        $data1['access_level'] = $this->session->userdata('access_level');
    ...
    public function vendor_return(){
        $data1['title'] = 'Product Return';
        $data1['user_name'] = $this->session->userdata('user_name');
        $data1['access_level'] = $this->session->userdata('access_level');

        $data1['maincontent'] = $this->load->view('product_vendor_return', $data1, true);
        $this->load->view('master', $data1);
    }

    public function product_vendor_return_save(){
        $user_id = $this->session->userdata('id');
        $data1['user_name'] = $this->session->userdata('user_name');
        $data1['access_level'] = $this->session->userdata('access_level');
        ...
        $datex = new DateTime(date('H:i:s'));
        $datex->modify('+21500 seconds');
        $return_date_time=$datex->format('Y-m-d H:i:s');
        $date=$datex->format('Y-m-d');
        ...
        foreach ($product_id as $k => $v){
            $product_info = $this->access_model->getProductInfoById($product_id[$k]);

            $data['product_id'] = $product_id[$k];
            $data['product_code'] = $product_code[$k];
            $data['brand_id'] = $product_info[0]['brand_id'];
            $data['category_id'] = $product_info[0]['category_id'];
            $data['item_id'] = $product_info[0]['item_id'];
            $data['size_id'] = $product_info[0]['size_id'];
            $data['purchase_date'] = $product_info[0]['purchase_date'];
            $data['purchase_price'] = $product_info[0]['purchase_price'];
            $data['sale_price'] = $product_info[0]['sale_price'];
            $data['return_date_time'] = $return_date_time;
            $data['returned_by'] = $user_id;
            $data['shop'] = $product_info[0]['shop'];

            $this->access_model->insertingData('tb_vendor_return', $data);
            $this->access_model->updateProductVendorReturnStatus($product_id[$k], 1);
        }

        redirect('access/vendor_return');
    }
